...
test("card subtitles use Russian plural forms", () => {
  const { dom } = setup();
  const doc = dom.window.document;
  assert.equal(doc.querySelector('[data-card-subtitle="site"]')?.textContent, "19 настроек");
  assert.equal(doc.querySelector('[data-card-subtitle="reader"]')?.textContent, "8 настроек");
  assert.equal(doc.querySelector('[data-card-subtitle="premium-free"]')?.textContent, "4 настройки");
});
