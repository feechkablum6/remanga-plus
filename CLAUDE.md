...
- **Settings peek button** — при `hideRightRail + minimizeSettingsButton` оригинальная кнопка настроек полностью скрыта (безусловно, в том числе при открытой панели). На её место ставится невидимая клонированная кнопка в зоне `settings-peek-zone` на правом краю. Зона — `pointer-events: none`, чтобы клики сквозь неё проходили к Full Screen и остальным кнопкам rail'а. Hover/focus ловит внутренний `settings-peek-content` (узкий регион у кнопки, `pointer-events: auto`) и запускает fade через `opacity`. Никаких `translateX`-трюков с торчащей полоской.
