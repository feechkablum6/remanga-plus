...
- **Settings peek button** — при `hideRightRail + minimizeSettingsButton` оригинальная кнопка настроек полностью скрыта (безусловно, в том числе при открытой панели). На её место ставится невидимая клонированная кнопка в зоне `settings-peek-zone` на правом краю. Зона — `pointer-events: none`, чтобы клики сквозь неё проходили к Full Screen и остальным кнопкам rail'а. Hover/focus ловит внутренний `settings-peek-content` (`pointer-events: auto`), который разворачивается на всю область rail'а **ниже** кнопки Full Screen (или на весь rail, если Full Screen скрыт) — это даёт широкий hover target, но не перекрывает FS. `align-items: flex-start` держит саму кнопку у верха content'а, в месте, где был оригинал настроек. Никаких `translateX`-трюков с торчащей полоской.
...
- [2026-04-17] Full Screen кнопка перестала кликаться при `minimizeSettingsButton`, потому что `settings-peek-zone` — fixed-блок 72px×rail-height с `pointer-events: auto` — перекрывал rail и ловил клики поверх Full Screen. → Зоне ставить `pointer-events: none`; hover/focus ловить на внутреннем `settings-peek-content` (`pointer-events: auto`), который узок и расположен ниже Full Screen.
- [2026-04-17] После перевода зоны в `pointer-events: none` peek-кнопка переставала появляться вне браузерного fullscreen: hover-target сжался до размеров самой кнопки (~48×48), и попадать в него мышью было тяжело; в fullscreen геометрия rail'а делала попадание более вероятным, создавая иллюзию «работает только в fullscreen». → `settings-peek-content` должен явно получать `width`/`height`, раздвигаясь на всю область rail'а ниже Full Screen кнопки (или весь rail, если FS скрыт). Чтобы кнопка при этом не растягивалась на всю высоту content'а, на `settings-peek-content` ставим `align-items: flex-start`.
- [2026-04-17] В браузерном fullscreen peek-кнопка визуально наезжала на Full Screen кнопку. Две причины: (1) позиционирование через `flex` + `align-items: flex-start` с клонированной кнопкой внутри оказалось нестабильным — клон наследует стили оригинала и может сдвигаться; (2) `fullscreenchange` не триггерил пересчёт peek-зоны, так что после входа в fullscreen координаты оставались от старой раскладки. → (1) `settings-peek-button` — `position: absolute; top: 0; right: 0` (детерминированное место внутри content, не зависит от flex-квирков клонированного DOM). (2) В `ensureFullscreenListener` после `syncFullscreenButtonStates()` вызывать `syncSettingsPeekZone` с последними настройками (сохраняются в module-level `lastSyncedSettings` при каждом `syncReaderEnhancer`).
