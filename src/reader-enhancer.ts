...
import {
  PREMIUM_FREE_READER_WIDTH_CLASS,
...
const syncPremiumFreePagerPages = (
  container: HTMLElement,
  ...
  renderPremiumFreePagerViewPages({
    container,
    result,
    state,
    currentPageIndex,
    onPageChange: (nextIndex) => {
      ...
      syncPremiumFreePagerPages(container, result, collectPremiumFreeReaderState(), key);
    },
  });
};
...
const renderPremiumFreePages = (
  ...
  if (readerState.mode === "pager") {
    resetPremiumFreeChapterStream();
    syncPremiumFreePagerPages(container, result, readerState, key);
    return;
  }
